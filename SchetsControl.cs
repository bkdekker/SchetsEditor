...
{   public class SchetsControl : UserControl
    {   private Schets schets;
        ...
        public void undo(object o, EventArgs ea)
        {
            ...
            this.Schoon(o, ea);
            if (huidigeTool.GetType() == typeof(GumTool))
            {
                Schets.elements_list = Schets.vorige_elementen_list;
                foreach (Element elem in Schets.elements_list) ;
            }
            else
            {
                int k = schets.elements_list.Count - 1;
                schets.elements_list.RemoveAt(k);
            }
            foreach (Element elem in Schets.elements_list)
            {
